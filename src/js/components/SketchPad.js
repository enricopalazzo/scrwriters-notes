...
class SketchPad extends React.Component {
  ...
  changeDot = (type) => {
    ...
    if (type === "erease") {
      signature.minWidth = 5;
      signature.maxWidth = 10;
      signature.style = { cursor: 'pointer' };
      this.setState({ "sizeofdot": 30 });
      this.setState({ "brushcolor": 'rgba(255, 255, 255, 0)' });
      signature.penColor = 'white';
      signature.dotSize = 500;

    }
    else {
      signature.minWidth = 0.5;
      signature.maxWidth = 2;
      signature.style = { cursor: 'crosshair' };
